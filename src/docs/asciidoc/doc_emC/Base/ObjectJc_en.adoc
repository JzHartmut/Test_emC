...
about the whole instance is primary not regarded. 
 ...
 class MyBaseClass: protected MyType_s      //contains ObjectJc as core
 {
   ...
   public: MyBaseClass();
   protected: MyBaseClass(ObjectJc* othiz);
 }
 ...
 { 
   ...
   protected: MyClass(ObjectJc* othiz);
 }
 
* The both classes can be instantiated, but it can be a base class of another derived one too.
Hence it has two constructors.

* The protected constructor which acts as base class constructor have an argument `ObjectJc* othiz`.
It is used to enable calling of `CTOR_ObjectJc(...)`. That is used for the `MyBaseClass`

* The public constructor which acts as instance constructor have not an `ObjectJc* othiz` argument
but it call `CTOR_ObjectJc(...)` in the argument preparation of the base constructor. 
Hence the initialization of the `ObjectJc` core part is done firstly. 

 MyClass::MyClass(int idObj) 
   //The public ctor without ObjectJc* argument presumes 
   //that the whole instance should be initialized:
 : MyBaseClass( //the initializer list can only call the base ctor:
                //but as argument the CTOR_ObjectJc is called. 
     CTOR_ObjectJc(this->base.obj, this, sizeof(MyClass), refl_MyClass, idObj))
 , objectJc(&this->base.obj)  //store the reference, is already initialized.
 { //...further init
 }
 
* A class which has not a base class but have a `ObjectJc const*` reference 
named `objectJc` should set it with the calling of the `CTOR_ObjectJc`:

 MyBaseClass::MyBaseClass() 
   //The public ctor without ObjectJc* argument presumes 
   //that the whole instance should be initialized:
 : objectJc( CTOR_ObjectJc(this->base.obj, this, sizeof(MyClass), refl_MyClass, idObj))
 { //...further init
 }
...
The const `objectJc` element allows read access to the `ObjectJc` core data.  
