...
=== Smoothing of the D part as general solution

If one have a sampled system, the changing of the input values from one to the next step
are sometimes less. This is typical, because the sampling rate of the controller
...
On the other hand for stabilization it is not necessary to regard very fast changes.
...
It means it is possible and recommended to combine the D-part with a smoothing.
...
----
  //                                   // == calculate D-Part
  float xz = thiz->xs;                 // last smoothed
  thiz->xs += f->fTsD * (wxd1 - xz);   // smooth the input
  float dx = thiz->xs - xz;            // the D part from smoothed input
...
As you see the differential part is limited. On fast change of the input
or on a reference value step the differential may be meanwhile to high, 
only then it is limited. 
...
In comparison to the solution without D-part, the integrator is faster (3 ms instead 4 ms)
and the P-gain is also higher (8.0 instead 4.0) because the instability without D-part
is solved by the damping effect of the D-part. The environment output x (red curve)
has not an overdrive.
 
image:../../img/Ctrl/crv_PID_stepResponse_Dpart_T3.png[]
...
But on an earlier time the controller output lefts the max value, 
because the negative D-part forces this limitation-left already on P-part = 2.0.
It means the integrator starts also a little bit earlier.
