...
because in a closed loop the value of `wx` should be in middle 0.
...
* 4) There are some discussions about "serial" and "parallel" form, 
whereas the shown serial from is more familiar in Europe. The "parallel" form is:

 y = kP * wx + ∫ wx/Ti * dt + dwx / Td )
   
The difference is only that the kP factor is used in the serial form for all parts.
...
----
    ...
    else {                             // integrate for next step only if not limited
      int32 wxP64i = (int32)(f->fI * wxP);       
      thiz->qI.qI32 += wxP64i;         // integer for exact integration.
      thiz->yIntg = (int32)((thiz->qI.qI32)) * thiz->par->fIy;  // float used. 
    }
----   

The wind-up preventing solution is very simple. It is combinded with the limitation.
...
For that the `PID*Ctrl_emC` has one input for the control error: `wx` 
for the PI control, and an extra input named `x` which can have the same value as `wx`, 
but can only connect to the controlled value `x`. From this input the D part is built.
...
can be visited in smoothed form (data viewing), and the gotten numbers to build
the D-part are in a similar range. 

The statements in C for the float variant are the following:

----
=>source: src_emC/emC/Ctrl/PIDf_Ctrl_emC.c
include::../../../../main/cpp/src_emC/emC/Ctrl/PIDf_Ctrl_emC.c[tag=stepD_PIDf_Ctrl_emC]
----
...
float fT1 = 0.01, fT2 = 0.02, fT3 = 0.02;  
---- 

In comparison to the solution without D-part, the integrator is slower (8 ms instead 4 ms)
but the P-gain is higher (8.0 instead 4.0) because the instability without D-part
is solved by the damping effect of the D-part. The environment output x (red curve)
has not an overdrive. This is also true on the non limited step from 0.5 to 0.4 right side.
 
image:../../img/Ctrl/crv_PIDf_stepResponse_Dwxh_T3.png[]


The resolution of the input signal is 12 bit, it is programmed as:

----
    float xquant = 2000.0f;
    float wx = ((int)(xquant*(wCtrl - xEnv)/valrange + 0.5f ) / xquant ) * valrange;
...
to compare it with the interger version of PID, but this is not important here.

The D part is the pink curve. First, it goes in the same direction as the P-part
because of the rising reference value. 
But after a small time, the rising of the input x value forces a negative value 
till max. 
But both are not effective because the controlling error forces a high P part. 
Hence also the integrator is hold, and the reaction of the system is firstly the same.

But the controller output lefts the max value on an earlier time , 
...
In this curve you see a little bit nosing on the D-part, and hence in the output.
This is smoothed in the system, but the actuator is restless. 
...
image:../../img/Ctrl/crv_PIDf_stepResponse_DwxhZ_T3.png[]

The P-Part (blue) is `(w-x) * kP`. Because `kP=8.0` and the visible step size is 4.0,
the input `x` is quantized with `0.5`. But the output is qunatized because of the P-part
with this visible step of `4.0` because of kP. This is 0.8% from the current value 500.0.
For the system this is not so an catastrophical problem, but the D part adds an additional restlessness.


What is the result of a only one stage smoothing, with double time:

----
float kP = 8.0f, Tn = 0.008f, Tsd1 = 0.0f, Tsd2 = 0.000400f, Td = 0.001500f;
float fT1 = 0.01, fT2 = 0.02, fT3 = 0.02;  
---- 

image:../../img/Ctrl/crv_PIDf_stepResponse_DwxhTs1Z_T3.png[]
...
image:../../img/Ctrl/crv_PIDf_stepResponse_DwxhavgZ_T3.png[]
...
to build the average. Depending on the possible smooth time of ..16*Tstep It needs 32 value buffer.
...
Compare this result with D-part without smoothing:

----
float kP = 8.0f, Tn = 0.008f, Tsd1 = 0.0f, Tsd2 = 0.0f, Td = 0.001500f;
float fT1 = 0.01, fT2 = 0.02, fT3 = 0.02;  
---- 

image:../../img/Ctrl/crv_PIDf_stepResponse_Dwxfasth_T3.png[]

Already in the unzoomed version you see terrible shocks on the controller output (actuator signal),
But the D-Part without smoothing does lesser oscillate. 
Compare the output under same conditions but with a 'theoretical' `float` input
of the system, or here simulated with 16 bit resolution on `wx`:

image:../../img/Ctrl/crv_PIDf_stepResponse_Dwxfastf_T3.png[]

Nice! That it is. You can present this result as simulation with 'digital twin' near the expectable results.
Your customer will be happy and agree the contract. 
But you have forgotten, you need a 16-bit DAC, and what is also: 
Technical signals may have some disturbance which can also filtered by the D-Part-smoothing.

The oscillation is away, because, of course, the smoothed D-part offers lesser phase adjusting.
That is a problem of mathematics.
...
with similar results as the float variant. The functionality is not extra described here.
...
----
 ...
 Last look on the step operation:
 
----
=>source: src_emC/emC/Ctrl/PIDi_Ctrl_emC.c
include::../../../../main/cpp/src_emC/emC/Ctrl/PIDi_Ctrl_emC.c[tag=step_PIDi_Ctrl_emC]
----
