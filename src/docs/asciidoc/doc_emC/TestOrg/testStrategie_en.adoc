...
This document describes general aspects of the source tree and test strategies
and also starting in chapter <<#buildTest>> the practial approach.
...
Similar to the guidelines for arrangement of files in maven or gradle, a directory tree has the following structure:
 ...
 ./ working tree
 +-.git/*      ... git archive in the working tree (not recommended)
 +-.git        ... alternatively .git file as link to the repository
 +-src-back    ... space for local copies, zips, (working also (temporary) without repository)
...
The time stamp in Unix (before Linux) and also in Linux have another approach as usual in Windows.
...
[#shell_script]
=== For MS-Windows: Asscociate extension .sh to unix-script.bat
...
[source,sh]
----
include::../../../../load_tools/windows_shellscriptBatches_template/unix-script.bat[]
----

Here you can simple change by comment lines, whether one of the following scripts is executed.

.sh: git-script.bat
[source,sh]
----
include::../../../../load_tools/windows_shellscriptBatches_template/git-script.bat[]
----

.sh: cygwin-script.bat
[source,sh]
----
include::../../../../load_tools/windows_shellscriptBatches_template/cygwin-script.bat[]
----

The major difference between `git-script.bat` and `cygwin-script.bat` is: 
...
.bom: ~/tools/tools.bom The "bill of material", bill of downloaded files with their version and check sum
[source,sh]
----
...
[source,sh]
----
include::../../../../load_tools/cmds_template/jztxtcmd.sh[]
---- 

This script calls purely only the class `org.vishia.jztxtcmd.JZtxtcmd` with the script to execute.


.sh: ~/cmds/javavishia.sh Shell script to execute a JZtxtcmd script
[source,sh]
----
include::../../../../load_tools/cmds_template/javavishia.sh[]
---- 

In the last script there are some things to adapt:
...
you can adapt the PATH for a JRE8, see link:#Unix_script[chapter For MS-Windows: Asscociate extension .sh to unix-script.bat]
...
Alternatively in Windows you can click `build.bat`. Note that nevertheless the shell script interpreter should be given,
internally `unix-script.bat` is called, which should use the Cygwin `sh.exe` and environment. 
See chapter <<#shell_script>>. 
...
=== Using a mirror location for git archives, setup of sources from mirror location
...
[source,filetree]
----
path/to/gitArchive/Test_emC
path/to/gitArchive/src_emC
----
...
Because the `./tools` are not able to found in this locations, you must first set up the tools and cmds
as described in chapter <<#cmds_tools>>. 
It presumes that the extension `.sh` on Windows is related to the shell script executer,
...
**Some scripts to deal with the archive**

You find a set of files hidden in `Test_emC/src/docs/gitMirrorArchiveScripts/*`
which should be copied on the root location of the both git archives 
(`path/to/gitArchive/Test_emC` and `{ddd}/src_emC`),
...
Do not copy this file into your working tree, 
then it may remove all your (changed and not commit) sources.
Thats why this script is not part of the git archive on the correct working position,
should be setup manually only for the archive position. 

Some scripts help to push, pop, work with tags. You can contribute to emC, if you get access rights.


**Copy from cloned mirror location**

If you have the files on the git archive directory with the proper time stamp, 
you can copy all files manually to your working location. Do not copy the `.git`.
...
[#manSrc]
=== Manually setup of sources from zip files

It is also possible to get the sources from a zip archive, also from the github:
...
Then you should get the both zips, also from src_emC **in the correct proper version**.
It is recommended to use the `./src-back` in the working tree for this zips.
Then you should unzip the content to the correct positions. The directory names should be proper, 
especially `src_emC` inside `src` should be used, not the name with date stamp from the zip form github.
You should be sure yourself which version you are working with.

Then you can afterwards get the git archives, setup the link from your working tree to the mirrored repository, 
set git to the correct version and commit. 
- Or alternatively compare with other given versions, save your work in a zip, update, try, get experience, ...

General it is recommended use both, backups (in zip) and version control:

* The version control with git is not the main choice for backup, it is more for history comarison.
* For data exchange with other people not using git (for example specific embedded projects), 
comparison with backups is a good idea.
* For long time archiving without a specific tool (some older version tools, 10 years ago, are yet outdated)
it is recommended using an ordinary backup of files.g
...
[#build]
=== start build and test for more tests
...
I have written a batch file which is associated to the extension .sh named unix-script.bat :
