...
[#shell_script]
=== For MS-Windows: Asscociate extension .sh to unix-script.bat, PATH extension to Cygwin (or MinGW) and Java 
...
I have written a batch file named `unix-script.bat` and associated it with the `.sh` extension with full content presented here:
...
echo -
echo unix-script.bat PATH_TO_SCRIPT [WORKING_DIR]
echo -
...
To simple execute unix (linux) shell scripts with the extension .sh 
you can write a batch file `unix-script.bat` , able to found in the PATH.
Templates of this files are stored in  
