...
The `./tools` directory in the working tree can anytime created by starting

`./src/load_tools/+loadTools.bat` or `~.sh` (Windows / Linux)

It is also possible to have a linked folder for tools, if the same tools are used in different locations,
for specific projects and also for Java developing with vishia sources and projects.
